function [Mo,Mi,Ho,Hi,Posep] = SIPSN(tmp_psi,RDC,rDC,R,Z,Coils)
%  SIPS Separatrix Inpolygon Points (only Separatrix plasma)
%  这里不考虑限制等离子体边界膨胀的限制器；仅包含内侧的线圈的限制器
%  Ver:1.07 update:2024/11/1 更新一个稳定的X点检索方法，这里限制X点的检索区域
%  Ver:1.08 update:2024/12/22 当检索到多个X点时选择通量最小的一个
...
indR1 = floor((max(max(cell2mat(Coils(2,1)))) - R(1,1))/dr) + 1;
indR2 = floor((min(min(cell2mat(Coils(3:end,1)))) - R(1,1))/dr) + 1;
indZ1 = floor((min(min(cell2mat(Coils(3:end,2)))) - Z(1,1))/dz) + 1;
indZ2 = floor((max(max(cell2mat(Coils(3:end,2)))) - Z(1,1))/dz) + 1;
