function [E1,R1] = GSSOR2(E0,RHS,R,Z,psibo,jbr,jbz,NIT,ome)
...
%                              dr2*RHS(NR-1,3:NZ-2) )/(2*(1+dr2/dz2));
    % ---- matrix
    %Psis_tmp(2:end-1,2:end-1) = ( (Psi_S0(3:end,2:end-1) + Psi_S0(1:end-2,2:end-1))/dr2 + ...
    %                              (Psi_S0(2:end-1,3:end) + Psi_S0(2:end-1,1:end-2))/dz2 - ...
    %                          0.5*(Psi_S0(3:end,2:end-1) - Psi_S0(1:end-2,2:end-1))./(R(2:end-1,2:end-1)*dr) )/(2/dr2+2/dz2);
    
    
    Psi_S0 = ome*Psis_tmp + (1 - ome)*Psi_S0;%改变边界
    for j = 1:length(psibo)                %update boundary value
        Psi_S0(jbr(j),jbz(j)) = psibo(j);
    end
    Psi_S0(1,:) = 0.0;
    iter = iter + 1;
end
